...
__global__ void gbufferToPBO(uchar4* pbo, glm::ivec2 resolution, GBufferPixel* gBuffer) {
    ...
    if (x < resolution.x && y < resolution.y) {
      ...
      //  float timeToIntersect = gBuffer[index].t * 256.0;
	    ...
	    glm::vec3 position = gBuffer[index].pos * 255.0f;
...
static glm::vec3* dev_denoised_temp = NULL;
...
void pathtraceInit(Scene* scene) {
	...
	cudaMalloc(&dev_denoised_temp, pixelcount * sizeof(glm::vec3));
    cudaMemset(dev_denoised_temp, 0, pixelcount * sizeof(glm::vec3));
...
__global__ void generateGBuffer (
  ...
  if (idx < num_paths)
  {
	...
	gBuffer[idx].pos = glm::normalize(shadeableIntersections[idx].position);   
  }
}
...
__global__ void ATrousFilter(glm::vec3* in, glm::vec3* out, GBufferPixel* gbuffer,
  glm::ivec2 resolution, float c_phi, float n_phi, float p_phi, float stepwidth) {
    ...
    glm::vec3 cval = in[x + y * resolution.x];
    glm::vec3 nval = gbuffer[x + y * resolution.x].nor;
    glm::vec3 pval = gbuffer[x + y * resolution.x].pos;

	float cum_w = 0.f;
    for (int i = -2; i <= 2; i++) {
        for (int j = -2; j <= 2; j++) {
            glm::ivec2 uv = glm::ivec2(x + j * stepwidth, y + i * stepwidth);
			uv = glm::clamp(uv, glm::ivec2(0, 0), resolution);

            glm::vec3 ctmp = in[uv.x + uv.y * resolution.x];
            glm::vec3 t = cval - ctmp;

            float dist2 = glm::dot(t, t);
            float c_w = min(exp(-dist2 / c_phi), 1.0);

            glm::vec3 ntmp = gbuffer[uv.x + uv.y * resolution.x].nor;
            t = nval - ntmp;
            dist2 = max(glm::dot(t, t)/ (stepwidth * stepwidth), 0.0);
            float n_w = min(exp(-dist2 / n_phi), 1.0);

            glm::vec3 ptmp = gbuffer[uv.x + uv.y * resolution.x].pos;
            t = pval - ptmp;
            dist2 = glm::dot(t, t);
            float p_w = min(exp(-dist2 / p_phi), 1.0);

            float weight = c_w * n_w * p_w;
			int k_idx = max(abs(i), abs(j));
            sum += ctmp * weight * kernel[k_idx];
            cum_w += weight * kernel[k_idx];
        }
    }
    out[x + y * resolution.x] = sum / cum_w;
}
...
void showDenoisedImage(uchar4* pbo, int iter, float c_phi, float n_phi, float p_phi, float filterSize) {
	...
	cudaMemcpy(dev_denoised_temp, dev_image, pixelcount * sizeof(glm::vec3), cudaMemcpyDeviceToDevice);

    int atrousIter = floor(log2(filterSize / 5.0));  
    for (int i = 0; i < atrousIter; i++) {
        float stepWidth =  pow(2, i);
        ATrousFilter <<< blocksPerGrid2d, blockSize2d >> > ( 
            dev_denoised_temp, dev_denoised_image, dev_gBuffer, 
			cam.resolution, c_phi, n_phi, p_phi, stepWidth);
        std::swap(dev_denoised_image, dev_denoised_temp);
    }
    // Retrieve image from GPU
