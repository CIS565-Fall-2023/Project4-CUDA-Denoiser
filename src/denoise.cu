...
__global__ void eawFilter(glm::ivec2 resolution, const glm::vec3* inImage, glm::vec3* outImage, SceneGbufferPtrs gBuffer, EAWParams params, bool edgeAwared, int iter, int luminDiv, Camera cam)
{
	...
	float stepSize = 1 << iter;
	if (x < resolution.x && y < resolution.y) {
		glm::vec3 color = glm::vec3(0.0f);
		const int kernSize = 5;
		float weightSum = 0.0f;
		int currIdx = x + y * resolution.x;
		glm::vec3 cColor = inImage[currIdx] / (float)luminDiv;
		glm::vec3 cAlbedo = gBuffer.dev_albedo[currIdx];
		glm::vec3 cPosition = util_get_world_position(glm::vec2(x, y), gBuffer.dev_z[currIdx], cam.position, cam.view, cam.up, cam.right, resolution, cam.pixelLength);
		glm::vec3 cNormal = util_oct_to_vec3(gBuffer.dev_normal[currIdx]);
		int ID = gBuffer.dev_primID[currIdx];
		/*if (cColor.x < 1e-6f && cColor.y < 1e-6f && cColor.z < 1e-6f)
		{
			outImage[currIdx] = cColor;
			return;
		}*/
		for(int dx=-2;dx<3;dx++)
			for(int dy=-2;dy<3;dy++)
			{
				...
				{
					glm::vec3 nCol = inImage[nIdx] / (float)luminDiv;
					glm::vec3 nAlb = gBuffer.dev_albedo[nIdx];
					glm::vec3 nPos = util_get_world_position(glm::vec2(x, y), gBuffer.dev_z[nIdx], cam.position, cam.view, cam.up, cam.right, cam.resolution, cam.pixelLength);;
					glm::vec3 nNorm = util_oct_to_vec3(gBuffer.dev_normal[nIdx]);
					int nID = gBuffer.dev_primID[nIdx];
					if (nID != ID) continue;
					float dist2;
					dist2 = glm::distance2(nCol, cColor);
					float cw = i == 0 ? 1.0 : min(exp(-dist2 * params.sigmaCol * stepSize), 1.0f);
					dist2 = glm::distance2(nAlb, cAlbedo);
					float aw = min(exp(-dist2 * params.sigmaAlbe * stepSize), 1.0f);
					float dPos = glm::distance2(nPos, cPosition);
					float pw = min(exp(-dPos * params.sigmaPos * stepSize), 1.0f);
					float dNorm = glm::distance2(nNorm, cNormal);
					float nw = min(exp(-dNorm * params.sigmaNorm * stepSize), 1.0f);
					float eaW = nw * pw * cw * aw;
					color += nCol * eaW * gaussian5x5[i];
					weightSum += eaW * gaussian5x5[i];
				}
			}
		outImage[currIdx] = color / (max(weightSum, 0.00001f));
	}
}
...
__global__ void svgfAccumulation(glm::ivec2 resolution, SceneGbufferPtrs gbuffer, SVGFBufferPtrs prevSVGFBuffer, SVGFBufferPtrs currSVGFBuffer, glm::vec3* radiance, int iter)
{
	...
	int currIdx = x + y * resolution.x;
	glm::vec3 albedo = gbuffer.dev_albedo[currIdx];
	glm::vec3 emission = gbuffer.dev_emission[currIdx];
	glm::vec3 currIllum = (radiance[currIdx] / (float)iter - emission) / max(albedo, glm::vec3(1e-3f));
	//float depth = gbuffer.dev_z[currIdx];
	

	//glm::vec3 currIllum = radiance[currIdx]/(float)iter;

	glm::vec3 prevIllum = glm::vec3(0.0);
	glm::vec2 prevMome = glm::vec2(0.0);
	float histLen = 0.0f;
	...
	currMome = currMome * alphaMome + prevMome * (1 - alphaMome);
	currIllum = currIllum * alphaLum + prevIllum * (1 - alphaLum);
	float sigma = glm::max(0.0f, currMome.y - currMome.x * currMome.x);
	currSVGFBuffer.dev_illum[currIdx] = glm::vec4(currIllum, sigma);
	currSVGFBuffer.dev_moments[currIdx] = currMome;
	currSVGFBuffer.dev_history_len[currIdx] = histLen;
}
...
void SVGFAccumulation(glm::ivec2 resolution, SceneGbufferPtrs gbuffer, SVGFBufferPtrs prevSVGFBuffer, SVGFBufferPtrs currSVGFBuffer, glm::vec3* radiance, int iter)
{
	...
	const dim3 blocksPerGrid2d(
		(resolution.x + blockSize2d.x - 1) / blockSize2d.x,
		(resolution.y + blockSize2d.y - 1) / blockSize2d.y);
	svgfAccumulation << <blocksPerGrid2d, blockSize2d >> > (resolution, gbuffer, prevSVGFBuffer, currSVGFBuffer, radiance, iter);
}
...
__global__ void svgfBilateralFilter(glm::ivec2 resolution, SceneGbufferPtrs gbuffer, SVGFBufferPtrs currSVGFBuffer, glm::vec4* filteredIllum)
{
	...
	else
	{
		...
		float luminC = util_math_luminance1(glm::vec3(illumC));
		...
		int idC = gbuffer.dev_primID[currIdx];
		float luminCoeff = LUMIN_COEFF;
		float depthCoeff = glm::max(znormalwidth.x, 1e-8f) * 3.0f;
		float normalCoeff = glm::max(znormalwidth.y, 1e-6f) * 3.0f;
		float totalW = 0.0f;
		...
		for(int dx=-3;dx<4;dx++)
			...
			{
				int nx = x + dx, ny = y + dy;
				int nIdx = nx + ny * resolution.y;
				int idN = gbuffer.dev_primID[nIdx];
				if (nx >= 0 && nx < resolution.x && ny >= 0 && ny < resolution.y)
				{
					glm::vec3 illumN = glm::vec3(currSVGFBuffer.dev_illum[nIdx]);
					float luminN = util_math_luminance1(illumN);
					...
					float w = util_compute_stopping_func(depthC, depthN, depthCoeff, normalC, normalN, normalCoeff, glm::vec3(illumC), illumN, luminCoeff, 1);
					totalIllum += w * illumN;
...
__global__ void svgfWaveletFilter(glm::ivec2 resolution, SceneGbufferPtrs gbuffer, SVGFBufferPtrs currSVGFBuffer, int iter, glm::vec4* filteredIllum)
{
	...
	float luminC = util_math_luminance1(glm::vec3(illumC));
	int idC = gbuffer.dev_primID[currIdx];
	float totalVar = 0.0f, totalW = 0.0f;
	for(int dx=-1;dx<2;dx++)
		for(int dy=-1;dy<2;dy++)
		{
			int nx = x + dx, ny = y + dy;
	...
	for(int dx=-2;dx<3;dx++)
		for (int dy = -2; dy < 3; dy++)
		{
			...
			if (nx >= 0 && nx < resolution.x && ny >= 0 && ny < resolution.y)
			{
				...
				float luminN = util_math_luminance1(glm::vec3(illumN));
				float depthN = gbuffer.dev_z[nIdx];
				glm::vec3 normalN = util_oct_to_vec3(gbuffer.dev_normal[nIdx]);
